...
file=open("Receiver_Output.txt","r")
received_data=ast.literal_eval(file.read())
...
serial_number=[]
file2=open("Receiver_Output2.txt","r")
sensor_data=ast.literal_eval(file2.read())
for i in range(len(device_number)):
    sensor_data[i]=ast.literal_eval(sensor_data[i])

Sensor_Information.SensorInformation(device_number,sensor_data)# Calculates the staus, temperature and other information regarding every sensor and stores in file
