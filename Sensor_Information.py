...
#        time.sleep(2) 
    for i in range(len(device_number)):
            ...
            serial_number=Device_Number.HexCombine([data_sensor[i][2],data_sensor[i][3],data_sensor[i][4]],3,16)
            ...
            file.write(str(device_number[i])+'\t\t'+device_type+'\t\t'+str(temp_time[3])+':'+str(temp_time[4])+'\t\t'+str(temperature_sensor)+'\t'+status_sensor+'\t'+str(status)+'\t'+str(device_number[i])+'-'+str(int(data_sensor[i][8],16))+'-'+str(int(data_sensor[i][9],16))+'\t\t'+str(device_signal[0])+'/'+str(device_signal[1])+'/'+str(device_signal[2])+'\n')
    else:
